...
def _run_brief_job(job_id: str, company: str) -> None:
    ...
    try:
        _ensure_ai_deps()
        from agent.business_explainer.graph import compile_app
        from agent.tools.data_fetcher import resolve_company
        from agent.tools.progress import bind, report as emit_progress, unbind
...
def start_business_brief(company: str) -> dict[str, Any]:
    ...
    from agent.tools.progress import init_business_explainer_agents
