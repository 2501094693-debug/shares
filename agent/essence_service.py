...
def _run_job(job_id: str, company: str) -> None:
    ...
    try:
        ...
        logger.exception("生意本质任务 %s 失败: %s", job_id, exc)
        job["status"] = "failed"
        job["error"] = str(exc)
        ...
        try:
            ...
            emit_progress("es_save", f"任务失败：{exc}", phase="failed", status="failed", level="error")
        except Exception:
            pass
    finally:
        try:
...
def start_essence_analysis(company: str) -> dict[str, Any]:
    company = (company or "").strip()
    if not company:
        raise ValueError("缺少公司名称或代码")
    if not os.getenv("OPENAI_API_KEY"):
        raise ValueError("未配置 OPENAI_API_KEY，无法启动生意本质")
    _ensure_ai_deps()

    from agent.tools.progress import init_essence_analyst_agents
    ...
    job = {
        ...
        "status": "running",
        "progress": [],
        "agents": init_essence_analyst_agents(),
        "activity_log": [],
        "current": None,
        "result": None,
        "error": None,
    ...
    threading.Thread(
        ...
        args=(job_id, company),
        daemon=True,
        name=f"ai-essence-{job_id}",
    ).start()
    return public_essence_job(job)


def list_essence_jobs(*, limit: int = 20) -> list[dict[str, Any]]:
    with _lock:
    ...
    return [
        public_essence_job(job, include_full_result=False)
        for job in (running + others)[: max(1, limit)]
    ]


def get_essence_job(job_id: str, *, include_full_result: bool = True) -> dict[str, Any] | None:
    with _lock:
        job = _jobs.get(job_id)
        if not job:
            return None
        snapshot = job
    return public_essence_job(snapshot, include_full_result=include_full_result)


def public_essence_job(job: dict[str, Any], *, include_full_result: bool = True) -> dict[str, Any]:
...
def read_essence_report(filename: str) -> str:
