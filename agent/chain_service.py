...
def _run_chain_job(job_id: str, company: str) -> None:
    job = _jobs[job_id]
    try:
        _ensure_ai_deps()
        ...
        logger.exception("产业链分析任务 %s 失败: %s", job_id, exc)
        job["status"] = "failed"
        job["error"] = str(exc)
        ...
        try:
            ...
            emit_progress("ch_analyze", f"任务失败：{exc}", phase="failed", status="failed", level="error")
        except Exception:
            pass
    finally:
        try:
...
def start_chain_analysis(company: str) -> dict[str, Any]:
    ...
    if not os.getenv("OPENAI_API_KEY"):
        raise ValueError("未配置 OPENAI_API_KEY，无法启动产业链分析")

    _ensure_ai_deps()

    from agent.tools.progress import init_chain_analyst_agents
    ...
    job = {
        ...
        "status": "running",
        "progress": [],
        "agents": init_chain_analyst_agents(),
        "activity_log": [],
        "current": None,
        "result": None,
        "error": None,
    ...
    thread = threading.Thread(
        target=_run_chain_job,
        args=(job_id, company),
        daemon=True,
        name=f"ai-chain-{job_id}",
    )
    thread.start()
    return public_chain_job(job)


def list_chain_jobs(*, limit: int = 20) -> list[dict[str, Any]]:
    with _lock:
    ...
    return [
        public_chain_job(job, include_full_result=False)
        for job in (running + others)[: max(1, limit)]
    ]


def get_chain_job(job_id: str, *, include_full_result: bool = True) -> dict[str, Any] | None:
    with _lock:
        job = _jobs.get(job_id)
        if not job:
            return None
        snapshot = job
    return public_chain_job(snapshot, include_full_result=include_full_result)


def public_chain_job(job: dict[str, Any], *, include_full_result: bool = True) -> dict[str, Any]:
...
def list_chain_reports() -> list[dict[str, Any]]:
    if not _REPORTS_DIR.exists():
        return []
    rows = []
    for path in sorted(_REPORTS_DIR.glob("*.md"), key=lambda p: p.stat().st_mtime, reverse=True):
        if not any(tag in path.name for tag in _CHAIN_REPORT_RE):
            continue
        stat = path.stat()
        rows.append(
            {
...
def read_chain_report(filename: str) -> str:
