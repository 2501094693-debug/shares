"""段永平看业务任务编排：后台运行 LangGraph，跟踪进度。"""
...
def _run_duan_job(job_id: str, company: str) -> None:
    job = _jobs[job_id]
    try:
        _ensure_ai_deps()
        ...
        job["result"] = {
            ...
            "numeric_hint": result.get("numeric_hint", ""),
            "business_verdict": result.get("business_verdict", ""),
            "culture_verdict": result.get("culture_verdict", ""),
            "price_verdict": result.get("price_verdict", ""),
            "final_attitude": result.get("final_attitude", ""),
        ...
        logger.exception("段永平看业务任务 %s 失败: %s", job_id, exc)
        job["status"] = "failed"
        job["error"] = str(exc)
        ...
        try:
            ...
            emit_progress("dy_synthesis", f"任务失败：{exc}", phase="failed", status="failed", level="error")
        except Exception:
            pass
    finally:
        try:
...
def start_duan_analysis(company: str) -> dict[str, Any]:
    ...
    if not os.getenv("OPENAI_API_KEY"):
        raise ValueError("未配置 OPENAI_API_KEY，无法启动段永平看业务")

    _ensure_ai_deps()

    from agent.tools.progress import init_duan_analyst_agents
    ...
    job = {
        ...
        "status": "running",
        "progress": [],
        "agents": init_duan_analyst_agents(),
        "activity_log": [],
        "current": None,
        "result": None,
        "error": None,
    ...
    thread = threading.Thread(
        target=_run_duan_job,
        args=(job_id, company),
        daemon=True,
        name=f"ai-duan-{job_id}",
    )
    thread.start()
    return public_duan_job(job)


def list_duan_jobs(*, limit: int = 20) -> list[dict[str, Any]]:
    with _lock:
    ...
    return [
        public_duan_job(job, include_full_result=False)
        for job in (running + others)[: max(1, limit)]
    ]


def get_duan_job(job_id: str, *, include_full_result: bool = True) -> dict[str, Any] | None:
    with _lock:
        job = _jobs.get(job_id)
        if not job:
            return None
        snapshot = job
    return public_duan_job(snapshot, include_full_result=include_full_result)


def public_duan_job(job: dict[str, Any], *, include_full_result: bool = True) -> dict[str, Any]:
...
def list_duan_reports() -> list[dict[str, Any]]:
    if not _REPORTS_DIR.exists():
        return []
    rows = []
    for path in sorted(_REPORTS_DIR.glob("*.md"), key=lambda p: p.stat().st_mtime, reverse=True):
        if not any(tag in path.name for tag in _DUAN_REPORT_RE):
            continue
        stat = path.stat()
        rows.append(
            {
...
def read_duan_report(filename: str) -> str:
