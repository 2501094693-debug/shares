"""发声散户情绪任务编排：后台运行 LangGraph，跟踪进度。"""
...
def _run_retail_job(
    ...
    *,
    ...
    try:
        ...
        logger.exception("发声散户情绪任务 %s 失败: %s", job_id, exc)
        job["status"] = "failed"
        job["error"] = str(exc)
        ...
        try:
            ...
            emit_progress("rs_synth", f"任务失败：{exc}", phase="failed", status="failed", level="error")
        except Exception:
            pass
    finally:
        try:
...
def start_retail_sentiment(
    ...
    *,
    ...
    from agent.tools.progress import init_retail_sentiment_agents
    ...
    job = {
        ...
        "status": "running",
        "progress": [],
        "agents": init_retail_sentiment_agents(),
        "activity_log": [],
        "current": None,
        "result": None,
        "error": None,
    ...
    thread = threading.Thread(
        target=_run_retail_job,
        args=(job_id, company),
        kwargs={
            "days": max(1, min(int(days or 3), 30)),
            "max_pages": max(1, min(int(max_pages or 3), 10)),
            "with_replies": bool(with_replies),
            "skip_llm": skip_llm,
        },
        daemon=True,
        name=f"ai-retail-{job_id}",
    )
    thread.start()
    return public_retail_job(job)


def list_retail_jobs(*, limit: int = 20) -> list[dict[str, Any]]:
    with _lock:
    ...
    return [
        public_retail_job(job, include_full_result=False)
        for job in (running + others)[: max(1, limit)]
    ]


def get_retail_job(job_id: str, *, include_full_result: bool = True) -> dict[str, Any] | None:
    with _lock:
        job = _jobs.get(job_id)
        if not job:
            return None
        snapshot = job
    return public_retail_job(snapshot, include_full_result=include_full_result)


def public_retail_job(job: dict[str, Any], *, include_full_result: bool = True) -> dict[str, Any]:
...
def list_retail_reports() -> list[dict[str, Any]]:
    if not _REPORTS_DIR.exists():
        return []
    rows = []
    for path in sorted(_REPORTS_DIR.glob("*.md"), key=lambda p: p.stat().st_mtime, reverse=True):
        if not any(tag in path.name for tag in _RETAIL_REPORT_RE):
            continue
        stat = path.stat()
        rows.append(
            {
...
def read_retail_report(filename: str) -> str:
