...
def _run_buffett_job(job_id: str, company: str) -> None:
    job = _jobs[job_id]
    try:
        _ensure_ai_deps()
        from agent.buffett_analyst.graph import compile_app
        ...
        logger.exception("巴菲特读表任务 %s 失败: %s", job_id, exc)
        job["status"] = "failed"
        job["error"] = str(exc)
        ...
        try:
            ...
            emit_progress("bf_synthesis", f"任务失败：{exc}", phase="failed", status="failed", level="error")
        except Exception:
            pass
    finally:
        try:
...
def start_buffett_analysis(company: str) -> dict[str, Any]:
    ...
    if not os.getenv("OPENAI_API_KEY"):
        raise ValueError("未配置 OPENAI_API_KEY，无法启动巴菲特读表")

    _ensure_ai_deps()

    from agent.tools.progress import init_buffett_analyst_agents
    ...
    job = {
        ...
        "status": "running",
        "progress": [],
        "agents": init_buffett_analyst_agents(),
        "activity_log": [],
        "current": None,
        "result": None,
        "error": None,
    ...
    thread = threading.Thread(
        target=_run_buffett_job,
        args=(job_id, company),
        daemon=True,
        name=f"ai-buffett-{job_id}",
    )
    thread.start()
    return public_buffett_job(job)


def list_buffett_jobs(*, limit: int = 20) -> list[dict[str, Any]]:
    with _lock:
    ...
    return [
        public_buffett_job(job, include_full_result=False)
        for job in (running + others)[: max(1, limit)]
    ]


def get_buffett_job(job_id: str, *, include_full_result: bool = True) -> dict[str, Any] | None:
    with _lock:
        job = _jobs.get(job_id)
        if not job:
            return None
        snapshot = job
    return public_buffett_job(snapshot, include_full_result=include_full_result)


def public_buffett_job(job: dict[str, Any], *, include_full_result: bool = True) -> dict[str, Any]:
...
def list_buffett_reports() -> list[dict[str, Any]]:
    if not _REPORTS_DIR.exists():
        return []
    rows = []
    for path in sorted(_REPORTS_DIR.glob("*.md"), key=lambda p: p.stat().st_mtime, reverse=True):
        if not any(tag in path.name for tag in _BUFFETT_REPORT_RE):
            continue
        stat = path.stat()
        rows.append(
            {
...
def read_buffett_report(filename: str) -> str:
