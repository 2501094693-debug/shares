...
def fetch_buffett_data(
    ...
    *,
    progress_node: str = "bf_fetch",
) -> dict[str, Any]:
    """采集巴菲特读表所需数据，并运行规则引擎预计算。"""
    ...
    report(node, f"采集 {name} 巴菲特读表数据", phase="fetch_data", status="running")
    ...
    report(node, "正在拉取最新全量定期报告（三大表 + 主要指标 + 业绩报表）…", phase="fetch_section")
    ...
    report(node, "正在运行巴菲特读表规则引擎…", phase="fetch_section")
    buffett = run_buffett_analysis(annual, recent, merged=merged)
    sections["巴菲特规则引擎预计算"] = buffett["text"]
    sources.append("规则引擎预计算")
    for title, table in (buffett.get("tables") or {}).items():
        sections[f"巴菲特_{title}"] = table
    ...
    report(node, "巴菲特读表数据采集与规则计算完成", phase="fetch_data_done", status="done")
