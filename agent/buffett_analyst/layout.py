...
SCAFFOLD_HEADINGS = {"解读", "原始数据", "本章小结", "综合判决"}
SECTION_PREFIXES = ("一、", "二、", "三、", "四、", "五、")
...
def render_section(title: str, table: str, content: str) -> str:
    ...
    parts.append("### 原始数据")
    parts.append("")
    parts.append((table or "").strip() or "（本章无独立数据表，依据规则引擎摘要与前述各章。）")
    parts.append("")
    parts.append("### 解读")
    parts.append("")
    parts.append(body)
