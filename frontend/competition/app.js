(() => {
  ...
  const REPORT_NAME_RE = /行业竞争分析/;
  const JOB_STORE_KEY = "orbit.competition.activeJob";
  const API_ROOT = "/api/ai/competition-analysis";
  ...
  async function revealCompleted(job) {
    ...
    try {
      ...
      $("reportMeta").textContent = "报告已生成，请点左侧历史报告打开";
    }
  }
  ...
  function renderResult(result) {
    ...
    $("reportTitle").textContent = `${result.stock_name || ""} 行业竞争分析${industry}`.trim();
  ...
  async function startAnalysis() {
    ...
    try {
      ...
      saveJobHandle(data);
      renderJob(data);
      $("reportTitle").textContent = `${company} · 行业竞争分析`;
      highlightActiveReport();
      startPoll();
    } catch (err) {
      showError(err.message);
  ...
  async function resumeActiveJob() {
    ...
    if (job.status === "running") {
      $("reportTitle").textContent = `${job.company || ""} · 行业竞争分析`.trim();
      startPoll();
      return true;
    }
  ...
  async function init() {
    ...
    try {
      ...
      showError(err.message || "恢复行业竞争分析任务失败");
    }
  }
