...
`orgId` 先查静态股票表，再对沪市 60/90、深市 000 做规则推断；002/300/688 等号段不规则，不能猜。请求走 `curl_cffi` 指纹时不要覆盖 User-Agent，否则巨潮 WAF 会 403，前端只能看到空列表。
...
1. 先查静态股票表（内存 / 本地缓存 / `szse_stock.json` 等），拿到真实 `orgId`
2. 表里没有时，只对沪市 60/90、深市 000 做规则推断；002/300/688 不要猜
3. 再走联想：`POST topSearch/query?keyWord=600519&maxNum=10`（WAF 可能 403，所以不能当第一手）
