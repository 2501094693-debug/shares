...
两步协议（不能只传 6 位代码）：
  ...
  2. hisAnnouncement/query 用 ``代码,orgId`` 分页拉列表
    ...
    python -m company.news.cninfo 600519
    python -m company.news.cninfo 600519 --days 90
    python -m company.news.cninfo 600519 --category annual
    python -m company.news.cninfo 600519 --keyword 问询函
    python -m company.news.cninfo 600519 --tab relation
    python -m company.news.cninfo 600519 --org-only
    python -m company.news.cninfo 000001 --download ./pdfs --limit 3
    python -m company.news.cninfo --market szse --category annual --days 30 --limit 20
"""
