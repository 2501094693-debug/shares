...
from analysis.livermore.config import CACHE_TAG, DEFAULT_LOOKBACK_DAYS
from analysis.livermore.screen import apply_view, screen_livermore
from analysis.persist import KIND_LIVERMORE, JobSlot, is_fresh, load_disk, save_disk, strip_charts


class LivermoreScreenService:
    def __init__(self) -> None:
        self._lock = threading.Lock()
        self._slots: dict[str, JobSlot] = {}

    @staticmethod
    def _make_key(days: int, code: str = "") -> str:
        return f"d{days}-c{code or '-'}-{CACHE_TAG}"
    ...
    def _snapshot(self, slot: JobSlot, top: int, kind: str) -> dict[str, Any]:
        elapsed = 0.0
        if slot.started_at:
            end = slot.finished_at or time.time()
            elapsed = round(end - slot.started_at, 1)
        payload: dict[str, Any] = {"status": slot.status, "elapsed_sec": elapsed}
        if slot.status == "error":
            payload["error"] = slot.error or "分析失败"
            return payload
        if slot.result is not None and slot.status in ("running", "done"):
            raw = slot.result
            payload["data"] = strip_charts(apply_view(raw, None if top <= 0 else top, kind))
        ...
        if slot.status == "running":
            ...
            if total:
                payload["message"] = f"已分析 {done}/{total}，按动作实时分类"
            else:
    ...
    def run_or_poll(
        ...
        *,
        ...
        key = self._make_key(days, code)
        with self._lock:
            slot = self._slots.get(key)
            if slot and slot.status == "running" and slot.run_id:
                return self._snapshot(slot, top, kind)
        ...
        packed = load_disk(KIND_LIVERMORE, key) if need_disk else None
        ...
        thread = threading.Thread(
            target=self._worker,
            args=(key, days, code, workers, run_id),
            daemon=True,
            name="livermore-screen",
        )
        thread.start()
        with self._lock:
            return self._snapshot(self._slots[key], top, kind)

    def _worker(self, key: str, days: int, code: str, workers: int, run_id: int) -> None:
        ...
        try:
            ...
            save_disk(KIND_LIVERMORE, key, result)
...
service = LivermoreScreenService()
