...
失败时返回空 dict，由 ``IndustryService.get_stock_profile`` 保留乐咕成分股字段兜底。
...
from data.stocks.common.fmt import fmt_list_date, fmt_pct, fmt_shares, fmt_yi_wan, to_float
...
def fetch_stock_quote(code: str, *, force: bool = False) -> dict[str, Any]:
    ...
    try:
        with ThreadPoolExecutor(max_workers=7) as pool:
            ...
            # 历史高低依赖上市年，所以等盘口回来再提交
            fut_ext = pool.submit(fetch_price_extremes, code, list_date)
            # 东财 f174/f175 是官方 52 周高低，自行计算的值不能盖掉它
            official_52 = {
    ...
    # 股息(TTM) 若只有股息率、没有每股股息，由 data_service 用现价 × 股息率补
    if result:
