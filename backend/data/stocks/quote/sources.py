...
``quote.fetcher.fetch_stock_quote`` 并行调用这里的函数，再拼成一张字段表。
...
from data.stocks.common.codes import em_code, secid, tencent_symbol
from data.stocks.common.fmt import (
    drop_empty,
    fmt_list_date,
    fmt_pct,
    fmt_price,
...
from data.stocks.common.http import get_json, get_text
...
def _returns_from_closes(closes: list[float], dates: list[str]) -> dict[str, Any]:
    """收盘价序列 → 3 日 / 5 日 / … / 今年以来涨幅。"""
    if len(closes) < 2:
        return {}
    last = closes[-1]
    out: dict[str, Any] = {}
    for key, days in _PERIOD_DAYS.items():
        if len(closes) > days:
            out[key] = pct_change(last, closes[-(days + 1)])
    ...
    if year:
        ...
        if ytd_base is not None:
            out["change_ytd"] = pct_change(last, ytd_base)
    return out
...
def _fetch_period_returns_tencent(code: str) -> dict[str, Any]:
    ...
    symbol = tencent_symbol(code)
    rows = fetch_tencent_day_rows(symbol, limit=320, qfq=False)
    closes: list[float] = []
    dates: list[str] = []
    for row in rows:
        ...
        closes.append(close)
    return _returns_from_closes(closes, dates)


def fetch_period_returns(code: str) -> dict[str, Any]:
    """区间涨幅。腾讯优先（东财 push2his 在不少网络会被掐），东财作补充。"""
    try:
        try:
        ...
        for line in klines:
            parts = str(line).split(",")
            # 东财 kline：date, open, close, high, low, ...（与 kline_fetcher 一致）
            if len(parts) < 5:
                continue
...
def fetch_f10_profile(code: str) -> dict[str, Any]:
    """注册资本、发行股本、自由流通股（流通股 − 持股≥5%，剔除港股通）。"""
    em = em_code(code)
    out: dict[str, Any] = {}
    headers = {
        "Referer": (
    ...
    try:
        ...
        if gbjg and isinstance(gbjg[0], dict):
            float_shares = to_float(
                gbjg[0].get("LISTED_A_SHARES") or gbjg[0].get("UNLIMITED_SHARES")
            )
        ...
        if float_shares is not None:
            ...
            if free > 0:
                out["free_float_shares"] = fmt_shares(free)
