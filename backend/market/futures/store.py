...
from core.paths import (
    ...
    FUTURES_TREE_CACHE,
    ensure_cache_dirs,
    futures_list_cache_path,
)
from market.futures import fetcher, taxonomy
...
class FuturesStore:
    ...
    def _load_index(self) -> None:
        ensure_cache_dirs()
        if not FUTURES_INDEX_CACHE.exists():
            return
        try:
            payload = json.loads(FUTURES_INDEX_CACHE.read_text(encoding="utf-8"))
    ...
    def _rebuild_search(self, items: list[dict[str, Any]]) -> None:
        ...
        for item in items:
            ...
            if not code:
                continue
            # 同代码跨所极少见；以 code 为键，后写覆盖
            key = code.upper()
            self._by_code[key] = item
            for i in range(1, len(key) + 1):
                self._code_prefixes[key[:i]].add(key)
            name = str(item.get("name") or "").strip()
            for ch in name:
                self._name_chars[ch].add(key)

    def _save_index(self) -> None:
        ensure_cache_dirs()
        FUTURES_INDEX_CACHE.write_text(
    ...
    def _write_category_cache(
        ...
        ensure_cache_dirs()
        futures_list_cache_path(category_code).write_text(
            json.dumps(payload, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
        return payload
    ...
    def _cache_fresh(self, payload: dict[str, Any] | None) -> bool:
        ...
        try:
            ...
            return False
        return (time.time() - ts) < FUTURES_LIST_TTL
    ...
    def get_category_list(
        ...
        *,
        ...
        if meta is None:
            raise KeyError(f"未知期货分类: {category_code}")
    ...
    def _merge_items(self, items: list[dict[str, Any]]) -> None:
        ...
        with self._lock:
            ...
            merged.sort(key=lambda x: str(x.get("code") or "").upper())
            self._index = {
                "updated_at": time.strftime("%Y-%m-%d %H:%M:%S"),
                "count": len(merged),
    ...
    def get_tree(self, *, force_refresh: bool = False) -> list[dict[str, Any]]:
        counts: dict[str, int] = {}
        if not force_refresh and FUTURES_TREE_CACHE.exists():
            try:
                cached = json.loads(FUTURES_TREE_CACHE.read_text(encoding="utf-8"))
    ...
    def search(
        ...
        *,
        ...
        if code:
            codes = self._code_prefixes.get(code, set())
            if not codes:
                codes = {c for c in self._by_code if c.startswith(code)}
        ...
        if venue:
            items = [
                item
                for item in items
                if str(item.get("venue") or "").upper() == venue
            ]
        if main_only:
            items = [item for item in items if item.get("is_main")]
    ...
    def start_build(self, *, force: bool = False) -> dict[str, Any]:
        ...
        if force or not self._index.get("complete"):
            thread = threading.Thread(
                target=_run, daemon=True, name="futures-index-build"
            )
            thread.start()
        else:
            with self._build_lock:
                self._building = False
        return {**self.status(), "building": bool(force or not self._index.get("complete"))}
